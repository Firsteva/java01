/* 상수 문자열
 ...
 각 클래스별로 상수문자열의 String 인스턴스가 준비된다.
 
 Method Area 영역에 각 클래스별로 상수 문자열을 준비하는 메모리 영역을
 상수풀(Constant Pool) 이라고 부른다.
 
 즉 Method Area 안에 클래스가 로딩되고,
 로딩된 클래스 안에 상수 문자열이 준비된다.
 
  * String Pool
 - 상수문자열을 보관하는 메모리영역
 - Method Area의 일부분이다.
 - 상수문자열은 자주 사용되기 때문에 메모리 낭비를 줄이기위해
   스트링 풀이라는 영역에 통합 관리한다.
 - 같은 상수문자열이 존재할 수 없다. => 중복으로 생성되지 않는다.
 */
...
public class StringTest02 {
  ...
  public static void main(String[] args) {
    String str1="Hello"; // String Pool에 인스턴스 생성 후 주소리턴
    String str2="Hello"; // 기존 인스턴스의 주소리턴
    String str3=new String("Hello"); // Heap에 인스턴스 생성

    if(str1==str2)System.out.println("str1==str2");
    if(str1==str3)System.out.println("str1==str3"); // 안됨
    // 자바는 문자열을 객체로 다룬다.
    // 문자열을 비교할 때는 equals()를 사용하라.
    
    // 객체이기때문에 == 연산자는 문자열이 아닌 주소를 비교하므로
    // 문자열 비교를 할 수 없다.
    if(str1.equals(str3))
